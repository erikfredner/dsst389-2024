...
Sentiment analysis attempts to determine how positive or negative the sentiment (mood) of a text is.
...
- But, we also intuit that "Dinner was catastrophic!" should be **more** negative than "Dinner was gross."
...
tidy_books |>
  inner_join(nrc |> filter(sentiment == "fear"), join_by(word)) |>
  count(word, sort = TRUE) |>
  slice_head(n = 3)
```
