...
corpus |>
  group_by(year, president) |>
  count(word) |>
  mutate(doc_len = sum(n), term_freq = n / doc_len) |>
  anti_join(stop_words, join_by(word)) |>
...
- $\text{df}$ is the number of documents in the corpus that contain at least one instance of the term
...
Almost all presidents change tax policies. Which are most associated with it?
