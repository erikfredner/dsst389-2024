...
Project 1 is due by the start of class on Monday, March 17.

## Data: Song lyrics

For this assignment, your goal is to *teach me something about Taylor Swift's lyrics* using data. This goal, like those of the other group projects, is open-ended by design.

I encourage you to bring your preexisting knowledge and preferences to this analysis!
...
These lyrics are stored as `.tsv` (tab-separated values) rather than the `.csv` (comma-separated values) files you are more familiar with. Rather than comma characters, these files use tabs to separate columns.
...
You may use my code below to get started with your analysis, but you don't have to.
...
### Make your own lyrics corpus!

The challenge for this assignment is to teach me something about Taylor Swift's lyrics. If you already know a lot about Swift, you may be able to do that by comparing different albums or groups of songs to one another.

Another way you may want to do that would be by comparing Swift to other artists whose songs you know well. The code chunk below creates a series of functions designed to scrape lyrics from [azlyrics.com](https://www.azlyrics.com) and combine them into a single tibble. You can read the comments to understand what the code is doing.
...
# Return a tibble with columns: artist, song, line_number, text, stanza, source_url
read_azlyrics <- function(url) {
  # Read HTML
  page <- read_html(url)

  # Parse the <title>, e.g. "Aesop Rock & Blockhead - Difficult Lyrics | AZLyrics.com"
  title_text <-
    page |>
    html_element("title") |>
    html_text2()
  ...
  if (length(right_divs) == 0) {
    stop("Could not find the expected <div> with lyrics comment. Layout may have changed.")
  }
...
This chunk below demonstrates how to use the functions above to scrape lyrics from three different Aesop Rock songs. You can modify this code chunk to choose any songs by any artist you like. If you plan to download a large number of songs to compare (e.g., 10+), I would recommend setting the `wait_sec` parameter to a larger number (e.g., 5 to 10) to avoid being blocked by the website.

Each of the URLs you select should be a link to the lyrics of a song on azlyrics.com. You can find these links by searching for the song on the website and copying the URL from the address bar. [Here is an example](https://www.azlyrics.com/lyrics/aesoprock/shrunk.html).
...
As with your previous assignment, the most important part of the project is the argument or "so what?" of your analysis.

A good argument tests existing assumptions. For example, many people associate Swift with breakup songs. It would be a good research question to figure out what proportion of Swift's songs discuss a breakup, and whether there are any characteristic features of breakup as compared to non-breakup songs. This could address the extent to which Swift's reputation is accurate.
...
All external resources must be cited in your final document. I do not require a formal citation style. However, if you want a model to follow, I recommend [The Chicago Manual of Style](https://richmond.primo.exlibrisgroup.com/permalink/01URICH_INST/191gg5k/alma9928619711206241).
...
| Analysis & Data Integration| Uses effective data manipulation and integration of relevant data sets (e.g., the Taylor Swift corpus, your own corpus). Briefly explains how you cleaned or manipulated data, references external resources and GenAI usage, and justifies decisions made regarding principle of selection. | 20%    |
...
| Code Quality & Style       | Uses well-documented, neat R code following tidyverse style conventions (pipes, naming, etc.). Incorporates advanced Quarto features (like figure captions, cross-references, or in-line calculations) where appropriate.                                                            | 10%    |
| Prose                      | Document includes at least 400 words describing the motivation, the analysis, and its findings. Is rendered as a professional Quarto PDF, with clear organization, headings, and formal writing style.          | 20%    |
